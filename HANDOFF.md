...
23. **🔴 回測窗口被暖身吃掉（2026-09-15 修正，所有舊回測數字都要重跑）**：
    ...
    **影響範圍：exit_buffer、take_profit、lynch×mid100「三關全過」等所有歷史結論
    ...
    ✅ 2026-09-16 重跑驗證：關2 空頭終於有交易了（基準回撤 -16.65%，不再是 0.00%），
    確認這就是「空頭壓測一直空過」的真正原因。**但所有修正前的結論都要重跑**——
    停利那份重跑後結果完全翻盤（見下方參數改動記分板），`exit_buffer` 那份也還沒重跑。
...
0. 🔴🔴 **最優先：全週期大盤對照**（2026-09-18）。重驗結果出來了，三組全部未通過，
   而且**多頭期全部輸給「直接買大盤」**：
   ```
   2024-01~2025-12    策略報酬   夏普    大盤 +62.23%（夏普 1.22）
   lynch × tw50       +45.25%   1.10    超額 -16.97%
   livermore × tw50   +36.06%   0.95    超額 -26.17%
   lynch × mid100     +58.51%   1.09    超額 -3.71%
   ```
   **大盤的夏普 1.22 比三組都高**——不是「用報酬換風險」的取捨，是兩邊都輸。
   ⚠️ 但這只是多頭那一半。防守型策略本來就該在多頭落後、在空頭保護，所以
   已在 `revalidate_deployed.py` 加上**全週期對照**（FULL = 2021-07~2025-12，
   跨 2022 空頭 + 2024~25 多頭）。**那個數字才是這個專案有沒有價值的判決**：
   取捨要成立，全週期至少要「報酬贏過買進持有」或「報酬接近但回撤明顯更小」，
   兩項都輸就代表犧牲沒換到任何東西。跑法見第 6 節。
   ✅ 空頭那一半的數字已經有了，**防守是真的**：
   ```
   空頭 2021-07~2022-12     報酬       回撤
   大盤                   -20.19%   -31.63%
   lynch × tw50              ?      -16.65%   ← 約大盤一半
   livermore × tw50          ?      -19.04%
   lynch × mid100            ?      -26.92%
   ```
   ⚠️ **但防守可能不是選股帶來的**：策略在空頭少賠，很可能只是因為大盤濾網讓它
   空手。所以 `revalidate_deployed.py` 又加了一個對照組 **「大盤+年線濾網」**
   （完全不選股，站上年線持有、跌破空手）。這是把**選股的價值**與**擇時的價值**
   分開的關鍵實驗：策略若贏不過這個對照組，代表財報、PEG、季線、停損、手續費與稅
   全都是白做的，真正在起作用的只有那條年線。
   ✅ **全週期結果出來了（2026-09-21）**：
   ```
   2021-07~2025-12        報酬      超額     夏普      回撤
   大盤買進持有          +63.51%      —     0.68   -31.63%
   lynch × tw50          +28.36%  -35.15%  0.43   -26.23%
   livermore × tw50      +58.13%   -5.38%  0.65   -24.52%
   lynch × mid100       +119.38%  +55.87%  0.92   -26.95%   ← 報酬、夏普、回撤全勝
   ```
   **同一個策略（lynch）換個股池，超額從 -35% 變成 +56%**，91 個百分點的落差。
   這跟停利驗證看到的「換股池就翻面」是同一種形狀，要非常小心。
   兩個可能：(a) GARP 本來就該打中小型股（林區自己的主張），(b) **mid100 生存者偏差**
   ——mid100 是今天還活著的名單，回測 2021 年等於預先知道誰撐過來了。
   🔴 **判決仍未定，因為最關鍵的對照組還沒跑出來**：
   - `大盤+年線濾網`（擇時的價值）→ 2026-09-18 那次**靜默失敗**沒印出來
     （TAIEX 逾時回 None 且被快取成哨兵）。已修成會講出原因。
   - `同股池等權買進持有`（選股的價值）→ 2026-09-21 新增。這一個最重要：
     跟 TAIEX 比會把「股池本身強不強」和「生存者偏差」算進策略功勞；
     跟**自己的股池**比，兩個因素兩邊都有、直接抵銷，剩下的才是選股的貢獻。
     mid100 的 +55.87% 若在這一欄變成負的，代表它贏的是股池不是選股。
   ✅ **對照組跑出來了（2026-09-22），這是目前最重要的一張表**：
   ```
   2021-07~2025-12          報酬     超額    夏普     回撤    選股貢獻
   大盤買進持有            +63.51%     —    0.68  -31.63%
   大盤+年線濾網           +68.81%  +5.31%  0.91  -18.69%   ← 不選股，只有擇時
     └ tw50 等權持有       +42.19% -21.32%  0.57  -26.72%
   lynch × tw50           +27.54% -35.96%  0.42  -24.78%   -14.64%
   livermore × tw50       +58.13%  -5.38%  0.65  -24.52%   +15.94%
     └ mid100 等權持有    +122.28% +58.77%  0.97  -38.36%
   lynch × mid100        +129.18% +65.67%  0.95  -32.46%    +6.90%
   ```
   **結論三點：**
   1. 🏆 **「大盤+年線濾網」報酬與夏普都贏過買進持有，回撤只有一半**，而且它
      不選股、不看財報、4.5 年只進出幾次。**它在風險調整後贏過三組策略中的全部**
      （夏普 0.91 vs 0.42/0.65/0.95，回撤 -18.69% 遠優於三組的 -24~-32%）。
      **這個專案絕大部分的價值就在那一條 200 日均線上。**
   2. mid100 的 +65.67% 超額**幾乎全是股池**（等權持有就有 +58.77%），
      選股只貢獻 +6.90%。之前以為挖到寶，其實挖到的是「中小型股這幾年很強」
      ＋生存者偏差。
   3. 意外：**livermore 的選股貢獻最高（+15.94%）**，正是我一度想砍掉的那個。
      但它仍輸給年線濾網（報酬/夏普/回撤三項全輸），且實盤紙上帳戶 -31%。
      lynch×tw50 則是唯一**選股為負（-14.64%）**——還不如把 tw50 買一籃子擺著。
...
1. ~~跑 `tools/revalidate_deployed.py` 重驗實盤三組設定~~（2026-09-17，✅ 已完成，結果見上）。
   ...
   真實基準是：tw50 WF 夏普 0.27、mid100 WF 夏普 0.29／空頭回撤 -26.92%——
   都低於既有工具早就寫死的 0.5 門檻。livermore 更是還沒用修好的引擎量過。
1. **三帳戶空跑觀察 2–4 週**：/holdings 看總帳，重點比較 lynch-mid100 vs 現任雙雄的實際表現。
   ⚠️ 空跑紀錄現在是**唯一沒有被暖身 bug 污染的證據**，份量比回測重。
2. ⛔ **上真錢的前提已消失，暫緩**：原計畫「空跑穩定後分配 5 萬到表現最好的組合」建立在
   錯誤的回測上。要繼續必須先過第 0 項，重新論證。（執行端改「盤後算訊號 → 隔日開盤送單」
   + 永豐實單審核仍是對的方向，但不是現在該推進的事。）
...
- **參數改動記分板**：lynch 固定停利 `take_profit`（tw50，2026-09-16 暖身修正後重跑）。
  **決議：維持 take_profit=0**（四個門檻全部未通過），但**理由與第一版完全不同**：
  ```
  版本        多頭報酬  多頭夏普  空頭回撤   WF夏普   WF報酬   勝率   最大單筆
  不停利        45.25%   1.10   -16.65%    0.27    3.04%   26%   +122.6%
  停利10%       61.86%   1.69   -22.38%    1.09   15.70%   40%    +20.0%
  停利15%       38.22%   1.13   -17.12%    0.14    1.18%   35%    +23.9%
  停利20%       17.23%   0.56   -10.65%    0.09    0.57%   26%    +28.4%
  停利30%       42.87%   1.14   -16.65%    0.50    6.56%   28%    +38.9%
  ```
  - 停利 10% **關1、關3 都大勝基準**，只卡在關2（空頭回撤 -22.38% vs -16.65%）。
    機制合理：停利在空頭裡把持股換成現金又追回去，來回挨刀、回撤放大。
  - **跨門檻忽好忽壞（10% 好、15% 普、20% 差、30% 普）→ 比較像路徑運氣而非結構性效果。**
    真實的效果應該隨門檻單調變化。這是不採用的主要依據，比「理論說砍上檔不好」硬。
  - ⚠️ **修正前那份結果（顯示單調惡化、報酬砍半）是錯的，已作廢**——它跑在被截斷的
    窗口上。當時我把它寫成「教科書般的結果」，那是**拿壞資料講一個很順的故事**。
    教訓：資料完美符合預期時，先去檢查資料，不要先慶祝。
  - 🔴 **順帶暴露的大問題：基準本身很弱**——不停利的 WF 夏普只有 0.27、WF 報酬 +3.04%，
    **低於本檔 `wf_min_sharpe=0.5` 的門檻**。也就是說「現在實盤在跑的這組設定，
    拿去過自己的關卡也會被刷掉」。lynch×tw50 的可信度要重新評估。
  - **mid100 跑完了，結論是 tw50 的鏡像（2026-09-17）**：
  ```
  版本        多頭報酬  多頭夏普  空頭回撤   WF夏普   WF報酬   勝率   最大單筆
  不停利        58.51%   1.09   -26.92%    0.29    4.59%   33%   +114.1%
  停利10%        8.49%   0.31   -23.61%   -0.23   -4.16%   37%    +19.8%
  停利15%       46.56%   1.06   -23.07%   -0.11   -2.49%   35%    +23.2%
  停利20%       19.99%   0.56   -24.55%    0.22    2.68%   31%    +31.6%
  停利30%       40.53%   0.90   -27.90%    0.08    0.22%   30%    +42.2%
  ```
  **同一個參數在兩個股池的效果完全相反**：tw50 的停利 10% 過關1、關3 卻卡關2；
  mid100 的停利 10% 卡關1、關3 卻過關2（空頭回撤反而改善）。而且 mid100 也是
  非單調的（8.49% → 46.56% → 19.99% → 40.53%，跳來跳去）。
  → **這是雜訊最典型的簽名**：真正的結構性效果不會換個股池就翻面。
  結論從「理論說砍上檔不好」升級成「資料本身不支持任何門檻」，證據力強得多。
  🔴 mid100 基準也一樣弱：WF 夏普 0.29、空頭回撤 **-26.92%**——
  而 lynch 當初的賣點正是「防守王，回撤僅 -7.2%」。那個賣點來自壞掉的引擎。
