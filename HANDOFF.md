...
TELEGRAM_CHAT_ID=...
DISCORD_WEBHOOK_URL=...         # Discord 頻道 Webhook（頻道設定→整合→Webhook→複製URL）
SHIOAJI_API_KEY=...
SHIOAJI_SECRET_KEY=...
...
⚠️ **雙向控制（/holdings /pause...）仍走 Telegram 監聽**，bot 凍結期間改用 SSH：
`cat runtime.json`（改 paused）、`cat paper_*.json`（看持倉）。Discord bot 監聽是待辦。
...
8. **測試**：`tests/` 共 90 個（下單路徑/fees/兩種 PaperBroker/多帳戶/策略/基準備援/心跳/通知多通道）。
...
4. （待辦）Discord bot 雙向控制（取代凍結的 Telegram listener）；或使用者申請新的 Telegram bot token。
