...
11. **FinMind 額度：基本面策略一天掃一次就好（2026-07-02 踩到）**：scan 一次要抓 tw50×5 種資料 ≈250 次
    FinMind 請求，每 5 分鐘掃會瞬間爆免費額度（"Requests reach the upper limit"）→ scan crash。
    lynch/buffett/graham 是**基本面**策略（PEG/EPS/營收是季月才變），timer 已改成**收盤後 14:00 一天一次**。
    只有動能突破策略（livermore/oneil）才需要盤中每 5 分鐘 +`--realtime`，且那時務必縮小 universe 或用付費 FinMind。
