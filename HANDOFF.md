...
- **待跑三關驗證**（回測+2022壓測+walkforward），贏過 livermore 才考慮部署：
  ```bash
  python main.py compare --strategy mclean,lynch,livermore --source finmind --universe tw50 --start 2023-07-01 --end 2026-07-01 --regime
  python main.py compare --strategy mclean,lynch,livermore --source finmind --universe tw50 --start 2021-07-01 --end 2024-07-01 --regime
  python main.py walkforward --strategy mclean --source finmind --universe tw50 --regime
  ```
...
8. **測試**：`tests/` 共 64 個（下單路徑/fees/兩種 PaperBroker/多帳戶/策略/基準備援）。
...
11. **FinMind 額度（滚動 60 分鐘窗口 600 請求，非整點重置）**：已用磁碟快取大幅降低用量
...
13. **mclean 若要部署**：法人資料 15:00–16:00 才公布，timer 應設 18:00 後（非 14:00）；回測用 T-1 籌碼比實盤保守，方向一致。

## 5. 下一步

1. **等 FinMind 額度寬鬆時跑 mclean 三關驗證**（指令在第 2 節），跟 lynch/livermore 同桌比。
...
python main.py compare --strategy mclean,lynch,livermore --source finmind --universe tw50 --start 2023-07-01 --end 2026-07-01 --regime   # 麥克連 vs 現任雙雄
...
├── data/             # sample(離線) / finmind(真實,含法人買賣超) / cache(記憶體+磁碟) / universe
...
- 新策略（含 mclean）一律先過三關再談部署；現行空跑的雙策略生產線不隨便動。別急著上真錢。
